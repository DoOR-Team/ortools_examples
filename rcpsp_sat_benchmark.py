...
PARSER.add_argument(
    '--input',
        default="/Users/jpbirdy/Workspaces/pyspace/cplex/ortools_demos"
                       "/rcpsp-or/data/rcpsp/single_mode/",
...
def solve_rcpsp(problem, proto_file, params):
    ...
    return solver.ObjectiveValue(), solver.WallTime()
...
def main(args):
    ...
    for f in tqdm(files):
        if f.startswith('.'):
            continue
        job_file = os.path.join(input_path, f)
        rcpsp_parser = pywraprcpsp.RcpspParser()
        rcpsp_parser.ParseFile(job_file)
        # print(rcpsp_parser.Problem())
        obj, walltime = solve_rcpsp(rcpsp_parser.Problem(), "", "")
        results.append({'file': f,
                        'objective': int(obj),
                        'walltime': walltime,
                        })
        with open('sm_result.json', 'w') as f:
            json.dump(results, f)
