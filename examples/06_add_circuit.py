...
def main(args):
    ...
    # 这个矩阵定义了，工作和工作之间切换需要的切换时间。
    # 正常情况下工作的切换不需要时间，如果工作的切换需要时间，需要考虑工作的链式结构
    # 此时需要使用AddCircuit链式约束
    # 链式约束，会确保链式定义的图中，至少保留一个哈密尔顿路径
    # 使用方式见下面
    setup_times = [
        [
    ...
    for i in all_jobs:
        ...
        for j in all_jobs:
            ...
            # 链式约束需要一个3元组(source_node, destination_node, literal)
            # 好像是确保一个哈密尔顿路径？
            # 利用这个literal，配合OnlyEnforceIf，强行将任务的先后顺序做好
